...
export default {
  ...
  computed: {
    ...
    maskStyle () {
      ...
      if (
        maskGradientColor.length === 1 ||
        maskGradientColor[0].indexOf('rgba') !== -1
      ) {
  ...
  methods: {
    init () {
      ...
      const initSelectedIndex = [
        ...this.localData.map((item, index) => {
          if (
            this.options['initSelectedIndex'] &&
            Array.isArray(this.options['initSelectedIndex'])
          ) {
    ...
    *asyncInit () {
    ...
    handleFilterData (index, expectItemIndex) {
      ...
      let _filterData =
        this.filterData === DEFAULT_DATA ? [...this.localData] : this.filterData
      ...
      for (let i = index; i < this.localData.length; i++) {
        if (i + 1 < this.localData.length) {
          ...
          const filterItem = next.filter((_item, _$index) => {
            ...
            if (
              _item['mapKey'] &&
              currentFilterData[expectItemIndexGroup[i]]['key']
            ) {
...
.picker-item-select {
  ...
  border-top: 1px solid $easychat-ui-border-dark;
  border-bottom: 1px solid $easychat-ui-border-dark;
}
