...
import {
  debounce as debounceFun,
  throttle as throttleFun,
...
export default {
  ...
  watch: {
    scrollTop (currentValue, prevValue) {
      ...
      if (
        this.isOnPullUpBackToBottom &&
        currentValue === -this._getScrollMaxHeight()
      ) {
  ...
  methods: {
    init () {
      ...
      this.bounceBackAniamte = new Animate(
        'bounce-back',
        bounceBackDuration,
        'ease-out',
      )
      ...
      this.scrollToAnimate = new Animate(
        'scroll-to',
        scrollToDuration,
        'ease-out',
      )
    ...
    _initDebounce () {
      ...
      if (debounce || throttle) {
        this.scrollEvent = debounce
          ? debounceFun(emitScroll, debounce.delay)
          : throttleFun(emitScroll, throttle.delay)
      } else {
    ...
    touchMoveHandler (e) {
      ...
      if (this.isOutOfRange) {
        this.scrollTop =
          scrollTop * DRAG_RATE +
          (this.scrollTop > 0 || this.scrollHeightMax < 0
            ? 0
            : -this.scrollHeightMax * (1 - DRAG_RATE))
      }
    ...
    touchEndHandler (e) {
      ...
      for (
        let i = endPos;
        i < position.length &&
        position[i].timeStamp > this.lastTouchTimeStamp - 100;
        i++
      ) {
        startPos = i
      }
      ...
      if (startPos !== endPos) {
        const timeOffset =
          position[endPos].timeStamp - position[startPos].timeStamp
    ...
    handlerOutOfRange () {
      ...
      if (this.scrollTop > 0) {
        ...
        this.handleBounceBackAnimate(
          this.scrollTop,
          this.scrollHeightMax < 0 ? 0 : -this.scrollHeightMax,
        )
      } else {
        return
      }
    ...
    velocityToAnimateScroll (velocityY) {
      ...
      this.scrollAnimate.start((next, value, now) => {
        ...
        if (
          interceptVelocityY &&
          Math.abs(this.decelerateVelocityY) <= interceptVelocityY
        ) {
        ...
        // 如果触摸到顶部或者底部，则完成动画
        if (
          Math.abs(this.decelerateVelocityY) < minVelocityToKeepDecelerating ||
          this.isOutOfRange
        ) {
    ...
    *pullUpGen () {
      const PULL_HEIGHT = this.localPullDown.height
      const SCROLL_MAX_HEIGHT = this._getScrollMaxHeight()
      yield this.handleBounceBackAnimate(
        this.scrollTop,
        -(SCROLL_MAX_HEIGHT + PULL_HEIGHT),
      )
    ...
    _getScrollMaxHeight () {
      ...
      const diff = parseInt(
        _boundClientRect.height - this._getContainerHeight(),
      )
