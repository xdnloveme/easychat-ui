  ...
  <transition name="fade" mode="out-in">
    <div v-show="isShow" class="toast-mask">
      <div
        ...
        :class="[
          'toast',
          showIcon ? '' : 'toast-no-icon',
          !message || message.length === 0 ? 'toast-no-msg' : '',
        ]"
      >
        <template v-if="showIcon">
          <img
            :class="['toast-img', styles.cssstyle ? cssAnalyze(styles) : '']"
            :src="iconSource"
          />
        </template>
        <p v-if="message && message.length > 0" class="toast-msg">
          {{ message }}
        </p>
      </div>
    </div>
